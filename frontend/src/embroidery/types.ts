...
/**
 * A single compiled machine instruction.
 * Generated by MachineCompiler from the full object list.
 *
 * This is what gets exported to DST/PES — the SAME data that drives rendering
 * (no separate fake-preview vs real-export split).
 */
export type MachineStitchType =
  | 'normal'       // regular needle penetration
  | 'tie-in'       // locking stitch at object start (very short)
  | 'tie-off'      // locking stitch at object end   (very short)
  | 'jump'         // travel stitch — needle up, machine moves
  | 'trim'         // thread cut command
  | 'color-change' // operator changes thread color

export interface MachineStitch {
  x:           number              // needle-down world X
  y:           number              // needle-down world Y
  type:        MachineStitchType
  color:       ThreadColor
  angleDeg:    number              // direction of stitch (degrees, 0 = right)
  lengthMm:    number              // stitch length in mm
  objectId:    string              // which EmbroideryObject this belongs to
  stitchIndex: number              // position within that object's sequence
  layerIndex:  number              // z-order (object index in sewing sequence)
}
